...
import { generateStudyPlan, generateResources, getStudyPlanHistory, getStudyPlanById, deleteStudyPlan } from '../utils/gemini';
...
export default function StudyPlannerPage({ language, progress, setProgress }) {
  ...
  // Load the most recent saved plan when the page first loads
  useEffect(() => {
    loadMostRecentPlan();
  }, []);
  ...
  const loadMostRecentPlan = async () => {
    try {
      const plans = await getStudyPlanHistory();
      if (plans && plans.length > 0) {
        const latest = plans[0];
        setPlan(latest.planData);
      }
    } catch {
      // silent fail — no history yet, or not logged in
    }
  };
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      {/* HEADER */}
      <div style={{ marginBottom: '28px', animation: 'fadeUp 0.4s ease', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
        <div>
        ...
        <button onClick={toggleHistory} style={{
          padding: '10px 20px', borderRadius: '12px', border: '2px solid #7c3aed',
          background: showHistory ? '#7c3aed' : 'white', color: showHistory ? 'white' : '#7c3aed',
      ...
      {/* HISTORY PANEL */}
      {showHistory && (
        <div style={{ background: 'white', borderRadius: '20px', padding: '24px', marginBottom: '24px', boxShadow: '0 4px 24px rgba(124,58,237,0.08)', border: '1px solid #ede9fe', animation: 'fadeUp 0.3s ease' }}>
          <h3 style={{ margin: '0 0 16px', fontWeight: '800', fontSize: '18px', color: '#1e293b' }}>Your Past Study Plans</h3>
          {historyLoading && <p style={{ color: '#64748b' }}>Loading...</p>}
          {!historyLoading && history.length === 0 && <p style={{ color: '#94a3b8' }}>No saved plans yet. Generate one below!</p>}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {history.map(p => (
              <div key={p._id} onClick={() => handleViewPlan(p._id)} style={{
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                padding: '14px 18px', borderRadius: '14px', border: '1px solid #ede9fe',
              ...
              >
                <div>
                  <p style={{ margin: '0 0 4px', fontWeight: '700', color: '#1e293b', fontSize: '15px' }}>{p.examName}</p>
                  <p style={{ margin: 0, color: '#94a3b8', fontSize: '12px' }}>
                    {p.subjects?.join(', ')} • {new Date(p.createdAt).toLocaleDateString()}
                  </p>
                </div>
                <button onClick={(e) => handleDeletePlan(p._id, e)} style={{
